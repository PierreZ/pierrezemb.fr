...
Your alphabet can include every interesting operation and still bury the bug, because enabling all of them in every run is its own failure mode. This is the insight behind [swarm testing](https://users.cs.utah.edu/~regehr/papers/swarm12.pdf), from Alex Groce, John Regehr, and colleagues at ISSTA 2012. Instead of running each seed with the full operation set at fixed weights, you enable a random subset of operations and disable the rest, then run thousands of seeds so you cover thousands of different subsets. In a week of testing, that found 42% more distinct ways to crash a collection of C compilers than the heavily hand-tuned default tester did. The clearest walk through why it works, with a production story behind it, is [Will Wilson's talk on swarm testing](https://www.youtube.com/watch?v=wzfC7Q-xNik), so go watch that. For a Rust workload, drive the subset choice from `context.rnd()` so each seed enables a different slice of your alphabet and stays reproducible.
...
These four patterns are easiest to see in one small workload. Picture a hash chain, a single piece of state holding `H`, the running digest of every block, and `N`, the number of blocks folded in so far. It starts empty at `H=0x0000, N=0`, and each `Append Block: b` operation folds the block into the digest with `H = hash(H, b)`, increments `N`, and emits an `AppendBlock<N, H>` event recording the new state.

The chain is a structural invariant in the same spirit as the Cycle workload above, because every block depends on the digest before it, so the whole structure has to reconstruct exactly. The emitted `AppendBlock<N, H>` events are the operation log, and in `check()` you replay them in order, recompute `H` and `N` from scratch, and compare against what FDB holds after all the chaos. That is also the conservation law, `N` has to equal the number of `AppendBlock` events and the recomputed digest has to equal the stored one, so nothing was lost, duplicated, or reordered. When they diverge, the first mismatching `N` points at the exact append that broke.
